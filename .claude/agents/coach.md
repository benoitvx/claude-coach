...
- `uv run claude-coach plan session add --plan-id N --date YYYY-MM-DD --sport <Run|Ride|Swim|TrailRun|VirtualRide|...> [--session-type endurance|threshold|intervals|long|race|recovery|renfo] [--duration <SECONDS>] [--distance <METERS>] [--intensity easy|moderate|threshold|vo2max|race] [--description "..."] [--notes "..."] [--blocks "<DSL>"]`
  - **Pour une séance vélo structurée** (HT/Zwift), renseigne `--blocks` avec le mini-DSL puissance (% FTP) → permet l'export `.zwo`. Segments séparés par `;` : `warmup:10m:50-65` (rampe), `40m@65` (steady), `3x[12m@95;4m@60]` (intervalles effort;récup), `cooldown:8m:65-50`. Ex : `--blocks "warmup:10m:50-65; 3x[12m@95;4m@60]; cooldown:8m:65-50"`. Réservé à la famille vélo.
- `uv run claude-coach plan session set-blocks <ID> "<DSL>"` — définit/remplace les blocs vélo d'une séance existante (même DSL que `--blocks`).
